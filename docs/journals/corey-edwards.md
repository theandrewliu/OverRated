...
right today was relatively focus on just a couple of things it started off with curtis being a real terrific guy and fixed my code issue that i was having because my tests were returning the right thing and it kept failing but he fixed it and explained it to me which then cause a chain of events where i helped jeremy fix the code that he was working on which was failing for the same reason and i also used that info as a helping hand towards leading yesenia and andrew into doing their tests too so we got all of those wrapped up today and then we started the process of us to begin deployment which started with refactoring and organizing our code thanks to black and flake8 to make sure everything was nice and neat and made sense and was still functional which is probably my favorite part about the code if im being honest and we also helped fix our ci-yml file with the help of curtis and our gitlab tests pass so now our last step for the last day will be deployment